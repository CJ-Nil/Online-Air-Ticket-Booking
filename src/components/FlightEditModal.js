...
function FlightEditModal({closeModal,id,setFlights}) {
    ...
    useEffect(() =>{
        ...
        axios.get('http://localhost:8080/on-air-mvc/Flights?id='+id+"&session="+sessionid)
            .then(response =>{
    ...
    function handleSubmit(e){
        e.preventDefault()
        const data = {
            id:id+"",from:from+"",to:to+"",cost:cost+"",depature:departure+"",arival:arival+"",airline:airline+"",session:sessionid+"",day:day+""
        }
        axios.post('http://localhost:8080/on-air-mvc/EditFlight',data)
            .then(response =>{
    ...
    return (
        <>
            <Modal.Header className='bg-info' closeButton>Edit Flight Details</Modal.Header>
            <Modal.Body>
            <Form onSubmit={handleSubmit} className="w-100 bg-info pt-2 pb-2 pl-4 pr-4 rounded">
                ...
                >Edit Flight Details</Form.Label>
                <Form.Group>
